...
const QuizComponent: React.FC<QuizComponentProps> = ({ question, answers, explanation }) => {
  ...
  const getAnswerClasses = (index: number) => {
    const baseClasses = "w-full text-left p-4 my-2 border rounded-lg transition-all duration-200 flex items-center";
  ...
  return (
    <Card className="!p-0 overflow-hidden">
          ...
          {answers.map((answer, index) => (
            ...
            >
              <span className="font-medium flex-grow">{answer.text}</span>
               {isSubmitted && answers[index].isCorrect && <CheckIcon />}
               {isSubmitted && index === selectedAnswer && !answers[index].isCorrect && <XMarkIcon />}
            </button>
          ))}
        </div>
        {!isSubmitted && (
      ...
      {isSubmitted && (
        <div className={`p-6 border-t ${isCorrect ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800'}`}>
            <h3 className={`text-lg font-bold flex items-center ${isCorrect ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200'}`}>
